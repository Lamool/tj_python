...
'''
    ...
    [3] 파싱 메소드, 속성
        1. .find(식별자, 속성명=값)    : 지정한 식별자가 속성명 동일한 태그 조회
        2. .select_one()
        3. .findAll() : 지정한 식별자가 일치한 여러 개 태그 추출해서 리스트로 변환
        4. .selelct()
            .태그명 : 해당 태그들의 첫 번째 태그 추출
            .text : 마크업 사이의 문자열 반한, 자식, 자손 가능, 주로 중첩텍스트일 때
            .string : 마크업 사이의 문자열, 자식만 가능, 자손 불가능, 주로 단일텍스트일 때
...
# [5] .findAll(식별자) : 지정한 식별자의 마크업 여러 개 조회하기
...
# [7] 반복문과 같이 활용
for div in htmlObj.select('div') :      # 모든 div를 추출해서 리스트 반환 한 다음 리스트 만큼 반복문 처리
    print(div.string)                   # div 하나씩 내용물 추출 1, 2, 3 다
