...
img_input = tf.expand_dims(img, 0)  # 0번 인덱스 (가장 앞에) 차원 추가 [700, 1200]  # 0은 0번 인덱스를 뜻함
print(img_input.shape)      # (1, 700, 1280, 3)

# 외부로부터 사전 학습된 모델 가져오기 # 쉅에서는 그래픽카드 사양의 한계로 CPU 사용함
...
# 3. 모델이 예측한 결과를 시각화 # 객체 탐지 결과를 시각화
boxes = result["detection_boxes"]               # Bounding Box 좌표 예측 값
labels = result["detection_class_entities"]     # 클래스(정답/종속) 아이디 # 분류 예측 값
...
for i in range(min(obj_to_detect, boxes.shape[0])) :
    if scores[i] >= 0.2 :       # 예측 신뢰도가 0.2(20%) 이상이면
        ...
        plt.plot([xmin, xmax, xmax, xmin, xmin], [ymin, ymin, ymax, ymax, ymin],
                 color='yellow', linewidth=2)       # 두께 : 2
        # 네 점의 좌표 찍기 : (xmin, ymin)왼쪽하단꼭짓점, (xmax, ymin)오른쪽하단꼭짓점, (xmax, ymax)오른쪽상단꼭짓점 ~~ 연결 경계선을 노란색
        # 4개의 좌푤를 그리고 마지막으로 첫 번째 좌표를 한 번 더 명시하여 마침을 한다. (다섯 번째 점이 첫 번째 점과 같아야 도형 닫기가 가능하다)
