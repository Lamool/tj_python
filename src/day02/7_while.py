...
# 예제1
treeHit = 0;    # 변수
while treeHit < 10 :    # 변수가 10미만이면 반복하고 아니면 반복을 종료한다는 뜻
    treeHit = treeHit + 1   # 파이썬에는 증감연산자가 없다. java/js 증감식 : treeHit++;
    print(f'나무를 { treeHit }번 찍었습니다.')   # f포매팅 : f'문자열{ 코드 }문자열'
    if treeHit == 10 :
        print('나무 넘어갑니다.')
# while, if 사용할 때 주의할점 : { } 없고 들여쓰기를 이용한 실행문들을 제어문에 포함하자


# (2) while문 만들기
number = 0              # 변수
while number != 4 :     # 변수가 4가 아니면 반복 하고 4이면 반복 종료하겠다는 뜻
    print("1.Add 2.Del 3.List 4.Quit")
    number = int (input())
    # input() : 콘솔창에서 입력받은 값을 반환해주는 함수
    # int() : 자료를 정수 타입으로 반환해주는 함수
# while, if : 들여쓰기 주의
...
while a < 10 :
        a += 1              # a의 값을 1 증가
        if a % 2 == 0 :     # a의 값이 짝수이면
            continue        # while문으로 이동한다, 아래 코든는 실행되지 않는다.
        print(a)            # 짝수는 출력되지 않고 홀수면 된다.
