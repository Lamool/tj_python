# 예외 처리란? 프로그램 실행 도중에 발생하는 오류를 다른 실행문으로 처리해주는 역할

list = [ 1, 2, 3 ]

# list[3] # 예외 발생 : 존재하지 않는 인덱스이므로 예외 발생
...
try :
    list[3]
except IndexError :     # 특정 예외를 처리 할 때는 예외클래스명 작성한다
    print("존재하지 않는 인덱스입니다 ")

# [3] 예외 처리 방법 3
    try:
        list[3]
    except IndexError as e :      # 특정 에외가 발생한 사유를 보고 싶을 때 as 예외 변수명 작성한다.    # list index out of range
        print(e)
