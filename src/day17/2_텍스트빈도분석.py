...
textData = re.sub(r'[^\w\s]', '', textData)
    # \w : 문자 혹은 숫자 의미   # \s : 공백(스페이스바, 탭) 의미   # ^ : 부정 의미
    # [^\w\s] : 문자, 숫자, 공백이 아닌 것 찾기
    # re.sub(r'정규표현식', '대체문자', '기존문자열')
    # 기존 문자열에서 정규표현식을 이용한 문자를 찾아 대체하는 함수
print(textData)
    # (3) 문자열을 토큰(단어)화
words = textData.split(" ")        # 문자열을 자를 때 사용 # 띄어쓰기 기준으로 문자 구분
print(words)

# 3. 문자 개수 세기
from collections import Counter     # 컬렉션(리스트/딕셔너리/집합)
wordCount = Counter(words) # 중복된 요소들의 개수를 튜플 형식으로 반환 해주는 함수
print(wordCount)
...
# (1) 워드클라우드 객체 생성 # WordCloud(width = 가로사이즈, height = 세로사이즈, background_color='배경색상').generare(시각화할문자열)
