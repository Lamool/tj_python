...
# (3) "문자열".index('찾을문자')
print(a.index('자'))     # 11, 문자열내 찾을 문자가 존재하면 인덱스 반환 함수
#print(a.index('가'))     # ValueError: substring not found, 만일 존재하지 않으면 예외 발생

# (4) "특정문자".join("문자열") : 문자열내 문자 사이에 특정 문자를 삽입해서 반환 함수
print(', '.join(a))     # 코, 딩, 도,  , 헤, 매, 는,  , 만, 큼,  , 자, 기,  , 땅, 이, 야
b = ["자바", "파이썬", "C언어", "자바스크립트"]
print('<->'.join(b))    # 자바<->파이썬<->C언어<->자바스크립트, 리스트 요소 마다의 특정 문자를 삽입해서 반환 함수

c = 'AbCd'
# (5) .upper() : 문자열내 소문자를 대문자로 치환해서 반환 함수
print(c.upper())
# (5) .lower() : 문자열내 대문자를 소문자로 치환해서 반환 함수
print(c.lower())
...
# (10) .replace('기존문자', '새로운문자') : 문자열내 기존 문자가 존재하면 새로운 문자로 치환해서 반환 함수
