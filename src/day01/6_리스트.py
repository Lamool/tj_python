...
#print(studentList[4])  # 예외 발생, IndexError: list index out of range
...
# (6) 리스트 관련 함수들
# 1. .append(요소값) : 리스트의 맨 마지막에 요소값 추가
a = [ 1, 2, 3 ]
a.append(4)
print(a)        # [1, 2, 3, 4]

# 2. .insert(인덱스, 요소값) : 리스트의 특정 인덱스에 요소값 추가
a.insert(0, 6)  # 0번 인덱스의 6 삽입, 삽입된 위치 뒤로 한칸씩 밀림.
print(a)        # [6, 1, 2, 3, 4]

# 3. .remove(삭제할 요소값) : 리스트의 삭제할 요소값이 존재하면 삭제
a.remove(1)
print(a)        # [6, 2, 3, 4]
# a.remove(1)     # 존재 하지 않는 값이면 예외 발생, ValueError: list.remove(x): x not in list
...
# 5. .count(찾을 요소 값) : 리스트의 해당 찾을 요소값이 존재하는 개수 반환
a = [ 1, 2, 3, 1 ]
print(a.count(1))       # 2, 리스트내 찾는 요소의 개수가 2 반환

# 6. .index(찾을 요소 값) : 리스트의 해당 찾을 요소 값이 존재하는 인덱스 반환
